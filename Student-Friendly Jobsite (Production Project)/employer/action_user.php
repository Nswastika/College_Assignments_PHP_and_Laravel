	...
	if(isset($_GET['sid'])){		
		...
		if($active==1){
            ...
            try {                                       
                ...
                $mail->Body    = 'Your account has been deactivated due to some reasons.';
                $mail->AltBody = 'Body in plain text for non-HTML mail clients';
                $mail->send();
                echo "<script>alert('Mail has been sent successfully!');window.location.href='students_manage.php';</script>";
            } catch (Exception $e) {
                echo "<script>alert('Message could not be sent. Mailer Error: {$mail->ErrorInfo}');window.location.href='students_manage.php';</script>";
            }					
		}else{
			//activate
            ...
            try {                                       
                ...
                $mail->Body    = 'Yor account is activated.';
                $mail->AltBody = 'Body in plain text for non-HTML mail clients';
                $mail->send();
