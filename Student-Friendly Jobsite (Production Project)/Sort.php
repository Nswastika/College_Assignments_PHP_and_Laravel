		...
		<form form method="POST" action="sort.php">
			<fieldset>
				 <?php
	       			...
	       			$conn = $pdo->open();
	       			$stmt = $conn->prepare("SELECT count(*) AS numrows FROM job, category WHERE  job.category_id = category.category_id AND  categoryname LIKE :keyword AND is_activated = '1'");
	       			...
	       			if($row['numrows'] < 1){
		       			...
		       			try{
		       			 	$inc = 3;	
						    $stmt = $conn->prepare("SELECT * FROM job, category WHERE  job.category_id = category.category_id AND  categoryname LIKE :keyword AND is_activated = '1'");
						    $stmt->execute(['keyword' => '%'.$_POST['keyword'].'%']);
					 
						    foreach ($stmt as $row) {
						    	$highlighted = preg_filter('/' . preg_quote($_POST['keyword'],'/') . '/i', '<b>$0</b>', $row['categoryname']);
								...
								<div class='company-list'>
								   ...
								   <div class='col-md-2 col-sm-2'>
									...
									<div class='col-md-2 col-sm-2'>
									    <button type='submit' class='btn view-job' name='View Job'><a style='color:white;' href='company-detail.php?id=".$row['job_id'].";'>View Job</a></button>
									</div>
								</div>																					
									";
	       						echo "</div>";
