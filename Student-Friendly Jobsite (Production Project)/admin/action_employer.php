	...
	if(isset($_GET['sid'])){		
		...
		if($active==1){
            ...
            try {                                       
                ...
                $mail->send();
                echo "<script>alert('Mail has been sent successfully!');window.location.href='manage_employers.php';</script>";
            } catch (Exception $e) {
                echo "<script>alert('Message could not be sent. Mailer Error: {$mail->ErrorInfo}');window.location.href='manage_employers.php';</script>";
            }					
		}else{
			//activate
